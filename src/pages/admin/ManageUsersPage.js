...
const ManageUsersPage = () => {
  ...
  const fetchUsers = () => {
    fetch('https://8x7r3mdp-3100.asse.devtunnels.ms/api/users')
  ...
  const handleUpdateUser = () => {
    fetch(`https://8x7r3mdp-3100.asse.devtunnels.ms/api/users/${editingUser.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const handleDelete = (userId) => {
    swal({
      ...
      if (willDelete) {
        axios.delete(`https://8x7r3mdp-3100.asse.devtunnels.ms/api/users/${userId}`)
          .then(() => {
