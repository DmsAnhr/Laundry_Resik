  ...
  const handleOrderSubmit = async (event) => {
    ...
    try {
      if (isCustomerSelected && selectedCustomerId) {
        ...
        await axios.delete(`https://8x7r3mdp-3100.asse.devtunnels.ms/api/orders/${selectedCustomerId}`);
        console.log('Order deleted successfully');
      }

      // Create a new order
      const response = await axios.post('https://8x7r3mdp-3100.asse.devtunnels.ms/api/orders', order);
